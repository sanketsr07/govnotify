...
const iN=(p)=>Math.ceil((new Date()-new Date(p))/86400000)<=3
...
export default function App(){
  ...
  // Bookmarks
  useEffect(()=>{
    if(user)fetch(`${API}/bookmarks/${user.token}`).then(r=>r.json()).then(d=>Array.isArray(d)&&setBm(d.map(j=>j.id))).catch(()=>{})
  },[user])
  ...
  const auth=async(type)=>{
    ...
    }catch{setErr("Server is waking up. Wait 30s and try again.")}
  ...
  if(pg==="login"||pg==="register"){
    ...
    return(
      <div style={{minHeight:"100vh",background:"#0c0c0f",display:"flex",alignItems:"center",justifyContent:"center",padding:16,fontFamily:"'Inter',system-ui,sans-serif"}}>
        <style>{`*{box-sizing:border-box;margin:0;padding:0}input::placeholder{color:#3f3f46}button:hover{opacity:.85}a{color:inherit}`}</style>
        <div style={{width:"100%",maxWidth:400}}>
          {/* Logo */}
          <div style={{textAlign:"center",marginBottom:32}}>
            <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:22,marginBottom:12}}>🇮🇳</div>
            <h1 style={{color:"#fff",fontSize:22,fontWeight:700,letterSpacing:-.5}}>{isReg?"Create account":"Welcome back"}</h1>
            <p style={{color:"#52525b",fontSize:13,marginTop:4}}>{isReg?"Free forever · No spam":"Sign in to your GovNotify account"}</p>
          </div>

          <div style={{background:"#18181b",borderRadius:16,padding:24,border:"1px solid #27272a"}}>
            {isReg&&<div style={{marginBottom:16}}>
              <label style={LS}>Full Name</label>
              <input placeholder="Sanket S R" value={form.n} onChange={e=>setForm({...form,n:e.target.value})} onKeyDown={e=>e.key==="Enter"&&auth(pg)} style={IS}/>
            </div>}
            ...
            <div style={{marginBottom:isReg?8:20}}>
              <label style={LS}>Password</label>
              <div style={{position:"relative"}}>
                <input placeholder={isReg?"Min 8 chars, A-Z, 0-9, !@#":"••••••••"} type={showP?"text":"password"} value={form.p} onChange={e=>setForm({...form,p:e.target.value})} onKeyDown={e=>e.key==="Enter"&&auth(pg)} style={{...IS,paddingRight:52,marginBottom:0}}/>
                <button onClick={()=>setShowP(!showP)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#52525b",cursor:"pointer",fontSize:11,fontWeight:700}}>{showP?"HIDE":"SHOW"}</button>
              </div>
            </div>

            {isReg&&form.p.length>0&&<div style={{marginBottom:16}}>
            ...
            {ok&&<div style={{background:"rgba(34,197,94,.1)",border:"1px solid rgba(34,197,94,.25)",borderRadius:8,padding:"10px 12px",marginBottom:14}}><p style={{color:"#86efac",fontSize:12}}>{ok}</p></div>}

            <button onClick={()=>auth(pg)} disabled={busy} style={{width:"100%",padding:12,borderRadius:10,background:busy?"#3f3f46":"#6366f1",color:"#fff",border:"none",fontSize:14,fontWeight:600,cursor:busy?"not-allowed":"pointer",marginBottom:16,transition:"background .2s"}}>
  ...
  if(pg==="profile"){
    return(
      <div style={{minHeight:"100vh",background:"#0c0c0f",fontFamily:"'Inter',system-ui,sans-serif"}}>
        <style>{`*{box-sizing:border-box;margin:0;padding:0}input::placeholder{color:#3f3f46}`}</style>
        {/* Nav */}
        <nav style={{padding:"0 20px",height:54,display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #18181b",background:"rgba(12,12,15,.95)",backdropFilter:"blur(10px)",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{maxWidth:580,margin:"0 auto",padding:"36px 16px 80px"}}>
          {/* Avatar + name */}
          <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:32}}>
            <div style={{width:56,height:56,borderRadius:16,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:24}}>
            {[{l:"Joined",v:prof?.joined||"—"},{l:"Saved Jobs",v:prof?.bookmarks??bm.length},{l:"Status",v:"Active"}].map(({l,v})=>(
              <div key={l} style={{background:"#18181b",border:"1px solid #27272a",borderRadius:12,padding:"14px 16px"}}>
          ...
          {/* Edit name */}
          <div style={{background:"#18181b",border:"1px solid #27272a",borderRadius:14,padding:20,marginBottom:12}}>
            <h2 style={{color:"#fff",fontSize:14,fontWeight:600,marginBottom:14}}>Profile</h2>
          ...
          <div style={{background:"#18181b",border:"1px solid #27272a",borderRadius:14,padding:20,marginBottom:12}}>
            ...
            {[{k:"o",ph:"Current password"},{k:"n",ph:"New password (8+ chars, A-Z, 0-9, !@#)"},{k:"c",ph:"Confirm new password"}].map(({k,ph})=>(
              <div key={k} style={{marginBottom:12}}>
                <input type="password" placeholder={ph} value={pwf[k]} onChange={e=>setPwf({...pwf,[k]:e.target.value})} onKeyDown={e=>e.key==="Enter"&&chPwd()} style={IS}/>
              </div>
            ))}
          ...
          {/* Sign out */}
          <div style={{background:"#18181b",border:"1px solid rgba(239,68,68,.15)",borderRadius:14,padding:20}}>
  ...
  // ── HOME ──────────────────────────────────────────────────
  return(
    <div style={{minHeight:"100vh",background:"#0c0c0f",fontFamily:"'Inter',system-ui,sans-serif",color:"#fff"}}>
      <style>{`
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          {user?(
            <>
              ...
              <button onClick={()=>setPg("profile")} style={{...NB,background:"rgba(99,102,241,.12)",color:"#818cf8",borderColor:"rgba(99,102,241,.25)",display:"flex",alignItems:"center",gap:5}}>
                <span style={{width:18,height:18,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700}}>
                  {user.name[0].toUpperCase()}
                </span>
                {user.name.split(" ")[0]}
              </button>
              <button onClick={logout} style={{...NB,color:"#52525b"}} className="desktop-only">Out</button>
            </>
          ):(
            <>
      ...
      {pg==="home"&&(
        <div style={{textAlign:"center",padding:"52px 20px 32px",position:"relative",overflow:"hidden"}}>
          ...
          <h1 style={{fontSize:window.innerWidth<640?32:44,fontWeight:900,letterSpacing:-2,lineHeight:1.08,marginBottom:12}}>
            Your next{" "}
          ...
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search any job..." style={{width:"100%",maxWidth:340,padding:"12px 18px",borderRadius:10,border:"1px solid #27272a",background:"rgba(255,255,255,.03)",color:"#fff",fontSize:14,outline:"none"}}/>
        </div>
      )}
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"0 14px 80px"}}>
        ...
        {/* Loading */}
        {pg==="home"&&load&&(
          <div style={{textAlign:"center",padding:"60px 0",color:"#27272a",fontSize:13}}>
            <p>Loading jobs...</p>
            <p style={{fontSize:11,marginTop:8,color:"#18181b"}}>Server may be waking up (30s)</p>
          </div>
        )}

        {/* Empty */}
        {pg==="home"&&!load&&fil.length===0&&<div style={{textAlign:"center",padding:"60px 0",color:"#27272a",fontSize:13}}>No jobs found</div>}
...
function JCard({j,i,bm,tog}){
  ...
  return(
    <div className="jc" onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{
      ...
      background:hov?cfg.color+"08":"rgba(255,255,255,.01)",
