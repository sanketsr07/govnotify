...
def register(data: RegisterSchema, db: Session = Depends(get_db)):
    ...
    user = models.User(name=data.name, email=data.email, password=hash_password(data.password))
    db.add(user)
    db.commit()
    db.refresh(user)
...
def get_profile(token: str, db: Session = Depends(get_db)):
    ...
    return {"name": user.name, "email": user.email, "joined": user.created_at.strftime("%B %Y"), "bookmarks": bookmark_count}
...
def add_bookmark(data: BookmarkSchema, db: Session = Depends(get_db)):
    ...
    if not payload:
        raise HTTPException(status_code=401, detail="Invalid token")
    exists = db.query(models.Bookmark).filter_by(user_id=payload["user_id"], notification_id=data.notification_id).first()
    if exists:
        db.delete(exists)
        db.commit()
